...
const ChessGameWrapper = (props) => {
  ...
  const [textAreaValue, setTextAreaValue] = React.useState(domainName + '/game/' + gameid);
  const [isCreator, setIsCreator] = React.useState(false);

  // Add a useEffect hook to check if the user is the game creator
  React.useEffect(() => {
  ...
  const handleStartGame = () => {
    socket.emit('startGame', { gameId: gameid, userName: props.myUserName });
  };
      ...
      {opponentDidJoinTheGame ? (
        <div>
          ...
          <div style={{ display: 'flex' }}>
            <ChessGame playAudio={play} gameId={gameid} color={color.didRedirect} isCreator={isCreator} />
          </div>
        ...
        <div>
          <h1 style={{ textAlign: 'center', marginTop: String(window.innerHeight / 8) + 'px' }}>
            Hey <strong>{props.myUserName}</strong>, copy and paste the URL below to send to your friend:
          </h1>
          <textarea
            style={{
            ...
            value={textAreaValue} // Use the state variable here
            onChange={() => {}} // This textarea is read-only, so we use an empty onChange function
            type="text"
          ></textarea>
          <br />
          {isCreator ? (
            <button onClick={handleStartGame} style={{ marginLeft: '50%', transform: 'translateX(-50%)', marginTop: '30px' }}>
              Start Game
            </button>
          ) : (
            <h1 style={{ textAlign: 'center', marginTop: '100px' }}>Waiting for the game creator to start the game...</h1>
          )}
        </div>
      )}
    </React.Fragment>
